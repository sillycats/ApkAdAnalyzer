...
[![Version](https://img.shields.io/badge/version-1.8-blue.svg)](https://github.com/sillycats/ApkAdAnalyzer/releases)
...
| 版本 | 说明 |
|------|------|
| **v1.8** | 新增远程授权（吊销）功能：授权配置 `auth_config.json` 位于仓库根目录，作者可远程控制授权、随时停止非法分发，原生层 HMAC-SHA256 签名校验防伪造；`native_crypto.cpp` 不再开源，编译为 `libnative_crypto.so`（arm64-v8a + armeabi-v7a）预置发布；广告特征库维持全加密产物（`ad_patterns_default.enc` / `online_ad_vendors.enc`），绝不明文开源 |
