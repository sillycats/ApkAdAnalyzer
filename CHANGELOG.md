...
### 新增
- **17 种语言切换**：在简体中文基础上新增繁体中文 + 16 种外文（英日韩、西葡德法意俄阿、印地印尼泰越马来乌克兰），共 18 种界面语言；支持"跟随系统"与手动切换，持久化选择。
- **标题/按钮多语言适配**：界面标题统一走多语言资源并随所选语言自动切换；标题与按钮文字支持自动换行，长语言文字不再被截断。
- **内置广告厂商特征库**：广告厂商特征（77 家厂商 / 5100+ 条）直接随 APK 内置（assets 混淆加密），全程离线加载，不发起任何网络请求。

### 变更
- 移除"同步在线广告厂商特征"菜单与远程在线获取逻辑（多镜像拉取 / 本地缓存），厂商特征完全改为内置 assets 直接调用。

### 修复
- **多次分析失败**：每次选择 APK 开始分析前自动清理历史分析缓存（`analysis_*` 临时文件与目录），可反复重新选择 APK 连续分析。
- **误导日志**：更新"未获取到在线广告厂商特征库(可在菜单手动同步)"为正确的内置特征库提示，全 18 语言同步。
